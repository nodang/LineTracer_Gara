		...
		case 2:
			...
			{ 
				...
				while(SW_U)
				{
					...
					{
					case 0:
						VFDPrintf("AC%6lu", (Uint32)(ACCELdivVEL_IQ10(MOTOR_SPEED_U32) >> 10));
						if(!SW_R)	{ DELAY_US(SW_DELAY);	TURN_ACCEL_U32 += 10; }
						else if(!SW_L)	{ DELAY_US(SW_DELAY);	TURN_ACCEL_U32 -= 10; }
						else;
						break;
					case 1:
						VFDPrintf("DC%6lu", (Uint32)(DECELdivVEL_IQ10(MOTOR_SPEED_U32) >> 10));
						if(!SW_R)	{ DELAY_US(SW_DELAY);	TURN_DECEL_U32 += 10; }
						else if(!SW_L)	{ DELAY_US(SW_DELAY);	TURN_DECEL_U32 -= 10; }
						else;
						break;
					}
					if(!SW_D)	
