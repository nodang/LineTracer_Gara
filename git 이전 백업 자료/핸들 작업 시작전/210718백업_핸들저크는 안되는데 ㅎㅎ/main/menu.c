		...
		case 1:
			...
			{ 
				...
				while(SW_U)
				{
					...
					{
					case 0:
						VFDPrintf("%2luAC%.3lf", (Uint32)ACCEL_COEF_U32, _IQ17toF(HANDLE_ACC_IQ17));
						if(!SW_R)	{ DELAY_US(SW_DELAY);	ACCEL_COEF_U32++; }
						else if(!SW_L)	{ DELAY_US(SW_DELAY);	ACCEL_COEF_U32--; }
						else;
						HANDLE_ACC_IQ17 = _IQ17(1.0) + _IQ17mpy(_IQ17(0.1), _IQ17(ACCEL_COEF_U32));
						break;
					case 1:
						VFDPrintf("%2luDC%.3lf", (Uint32)DECEL_COEF_U32, _IQ17toF(HANDLE_DEC_IQ17));
						if(!SW_R)	{ DELAY_US(SW_DELAY);	DECEL_COEF_U32++; }
						else if(!SW_L)	{ DELAY_US(SW_DELAY);	DECEL_COEF_U32--; }
						else;
				...
				//ACCEL_COEF_U32 = DECEL_COEF_U32 = 0;
				save_accel_rom();
				DELAY_US(SW_DELAY);
			}
			else;
