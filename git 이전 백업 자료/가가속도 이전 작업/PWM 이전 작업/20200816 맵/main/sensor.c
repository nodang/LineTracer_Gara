...
interrupt void SENSOR_ISR() 
{
	...
	if(Flag.Sensor_U16 == ON)	GpioDataRegs.GPASET.all = sensor_prog[SENSOR_COUNT];
	else;
...
void POSITION_COMPUTE()
{
	...
	if(sum_127div_u16)
	{
		...
		if(Flag.Cross == ON)
		{
			SenAdc.Position_IQ10 =	(SenAdc.Position_IQ10 > _IQ10(POSITION_WEIGHT_I32[7])) ? _IQ10(POSITION_WEIGHT_I32[7]) :
									(SenAdc.Position_IQ10 < _IQ10(POSITION_WEIGHT_I32[8])) ? _IQ10(POSITION_WEIGHT_I32[8]) : SenAdc.Position_IQ10;
		}
		else;
...
void START_END_LINE()
{
	...
	else if((Flag.MoveState_U16 == ON) && (TIME_INDEX_U32 > (PERIOD_1div4SENCONDdivCPUTIMER >> 17)))	
