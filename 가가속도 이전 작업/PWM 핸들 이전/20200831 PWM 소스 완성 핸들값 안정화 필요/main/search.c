...
void Init_RUN()
{
	...
	EPwm1Regs.TBPRD = (Uint16)MOTOR_PERIOD_LIMIT;
	EPwm2Regs.TBPRD = (Uint16)MOTOR_PERIOD_LIMIT;
}
...
void RUN()
{
	...
	//RMotor.Handle_IQ28 = _IQ28(1.0);
	//LMotor.Handle_IQ28 = _IQ28(1.0);
	while(1)
	{
		//TxPrintf("%u\n", LINE_OUT_U16);
		//TxPrintf("%ld %ld %ld %ld %ld %ld\n", LMotor.PrdHandle_IQ13 >> 13, RMotor.PrdHandle_IQ13 >> 13, LMotor.PrdNext_IQ14 >> 14, RMotor.PrdNext_IQ14 >> 14, LMotor.NextVelocity_IQ15 >> 15, RMotor.NextVelocity_IQ15 >> 15);
		...
		if(!LINE_OUT_STOP())		return;
		else;
		if(Flag.STOP & ((LMotor.Velocity_IQ15 <= _IQ15(MIN_VELO)) && (RMotor.Velocity_IQ15 <= _IQ15(MIN_VELO))))		
		{	END_STOP();				return;	}
		else;
	} 
}
