...
void save_accel_rom()
{
	...
	save_rom[ j++ ] = ( (Uint16)ACCEL_COEF_U32 >> 0 ) & 0xff;
	save_rom[ j++ ] = ( (Uint16)ACCEL_COEF_U32 >> 8 ) & 0xff;

	save_rom[ j++ ] = ( (Uint16)DECEL_COEF_U32 >> 0 ) & 0xff;
	save_rom[ j++ ] = ( (Uint16)DECEL_COEF_U32 >> 8 ) & 0xff;
...
void load_accel_rom()
{
	...
	ACCEL_COEF_U32 = ( ( load_rom[ j++ ] & 0xff ) << 0 );
	ACCEL_COEF_U32 |= ( ( load_rom[ j++ ] & 0xff ) << 8 );

	DECEL_COEF_U32 = ( ( load_rom[ j++ ] & 0xff ) << 0 );
	DECEL_COEF_U32 |= ( ( load_rom[ j++ ] & 0xff ) << 8 );
}
