...
{	
	...
	RMotor.ErrorVelocity_IQ17 = (_IQ17abs(RMotor.TargetVel_IQ17 - RMotor.NextVelocity_IQ17) >> 1);
	LMotor.ErrorVelocity_IQ17 = (_IQ17abs(LMotor.TargetVel_IQ17 - LMotor.NextVelocity_IQ17) >> 1);

	RMotor.Jerk_IQ16 = LMotor.Jerk_IQ16 = _IQ16div(jerk, _IQ16(10000.0));
...
void LINE_DIVISION(TRACKINFO *LINE, Uint16 cnt)
{
	...
	{	
		if(LINE->TurnDir_U32 & LARGE_TURN)		TURN_DIVISION(LINE, cnt);
		else	{		LINE->Velo_IQ17 = LINE->VeloOut_IQ17 = LINE->VeloIn_IQ17 = _IQ17(SECOND_CURVE_U32);		LINE->Jerk_IQ16 = _IQ16(JERK_U32);		}
	}
}
...
void STRAIGHT_DIVISION(TRACKINFO *LINE, Uint16 cnt)
{
	...
	{
		LINE->VeloOut_IQ17 = _IQ17(MOTOR_SPEED_U32);			
		(LINE + 1)->VeloIn_IQ17 = _IQ17(0.0); 								// If it's End Turnmark, Next enter-velocity and escape-velocity is Zero
	}
	if(LINE->Distance_U32 > LONG_DIST)			LINE->Jerk_IQ16 = _IQ16(JERK_LONG_U32);
	else if(LINE->Distance_U32 > MID_DIST)		LINE->Jerk_IQ16 = _IQ16(JERK_MIDDLE_U32);
	else if(LINE->Distance_U32 > SHORT_DIST)	LINE->Jerk_IQ16 = _IQ16(JERK_SHORT_U32);
	else										LINE->Jerk_IQ16 = _IQ16(JERK_U32);
	...
	if(LINE->MotorDistance_IQ17 >= _IQ17(LINE->Distance_U32))													// If compute-distance is more than total-track-distance
	{
		...
		VEL_COMPUTE(_IQ17(LINE->Distance_U32), _IQ17(0.0), low_vel, LINE->Jerk_IQ16, &LINE->Velo_IQ17);
	...
	{
		VEL_COMPUTE(_IQ17(LINE->Distance_U32), LINE->MotorDistance_IQ17, high_vel, LINE->Jerk_IQ16, &LINE->Velo_IQ17);
...
void TURN_DIVISION(TRACKINFO *LINE, Uint16 cnt)
{
	...
	LINE->VeloIn_IQ17 = LINE->VeloOut_IQ17 = _IQ17(SECOND_CURVE_U32);	LINE->Jerk_IQ16 = _IQ16(JERK_U32);
	if((LINE - 1)->TurnDir_U32 & LARGE_TURN)		LINE->VeloIn_IQ17 = (LINE - 1)->VeloOut_IQ17;
	...
	if(LINE->MotorDistance_IQ17 >= _IQ17(LINE->Distance_U32))													// If compute-distance is more than total-track-distance
	{
		LINE->DecelDistance_IQ17 = _IQ17(LINE->Distance_U32);													// decel-distance substitute total-track-distance
		VEL_COMPUTE(_IQ17(LINE->Distance_U32), _IQ17(0.0), low_vel, LINE->Jerk_IQ16, &LINE->Velo_IQ17);
		if(((LINE + 1)->TurnDir_U32 & LARGE_TURN) || ((LINE + 1)->TurnDir_U32 & STRAIGHT))	LINE->VeloOut_IQ17 = LINE->Velo_IQ17;
	}
	else
	{
		VEL_COMPUTE(_IQ17(LINE->Distance_U32), LINE->MotorDistance_IQ17, high_vel, LINE->Jerk_IQ16, &LINE->Velo_IQ17);
