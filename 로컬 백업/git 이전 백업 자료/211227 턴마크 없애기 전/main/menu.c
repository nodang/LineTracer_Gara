...
static void POS()
{
	...
	while(SW_U)
	{
		...
		Flag.Cross = OFF;
		POSITION_COMPUTE(&SenAdc);
		TURN_DECIDE(&RMark, &LMark);
		TURN_DECIDE(&LMark, &RMark);
		TxPrintf("P : %ld | H : %ld | C : %u | L:%u R:%u C:%u\n", (int32)(SenAdc.Position_IQ10 >> 10), (int32)(HanPID.Pos_PID_IQ17 >> 17), SenAdc.Position_U16_CNT, LMark.TurnFlag_U16, RMark.TurnFlag_U16, Flag.Cross);
...
void MENU_SENSOR()
{
	...
	{	
		...
		case 0:
			VFDPrintf("MAX||MIN");
			if(!SW_D)	{ DELAY_US(SW_DELAY);	Flag.Sensor_U16 = ON;	
												SENSOR_MAXMIN();	
												Flag.Sensor_U16 = OFF;
						GpioDataRegs.GPACLEAR.all = SENall; }
			else;
...
void MENU_RUN()
{
	...
	{	
		...
		case 2:
			VFDPrintf("3TH RACE");
			if(!SW_D)	{ DELAY_US(SW_DELAY);	RUN_THIRD();	while(SW_U);	DELAY_US(SW_DELAY); }
			else;
			break;
		case 3:
			VFDPrintf("mapCHECK");
			if(!SW_D)	{ DELAY_US(SW_DELAY);	LINE_PRINTF(); }
			else;
			break;
		}
