...
void save_mark_rom()
{
	...
	mark_rom[ i++ ] = ( ( ( Uint16 )g_u16_turnmark_cnt - 1) >> 0 ) & 0xff;
	mark_rom[ i++ ] = ( ( ( Uint16 )g_u16_turnmark_cnt - 1) >> 8 ) & 0xff;
...
void load_mark_rom()
{
	...
	g_int32total_cnt = ( int32 )( ( mark_rom[ i++ ] & 0xff ) << 0 );
	g_int32total_cnt |= ( int32 )( ( mark_rom[ i++ ] & 0xff ) << 8 );
...
void save_line_info_rom( void )
{
	...
	for( i = 0; i < 128; i++ )
	{
		dist_rom[ j++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32dist ) ) >> 0  ) & 0xff;
		dist_rom[ j++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32dist ) ) >> 8  ) & 0xff;

		turn_rom[ k++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32turn_way ) ) >> 0  ) & 0xff;
		turn_rom[ k++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32turn_way ) ) >> 8  ) & 0xff;

		ldist_rom[ l++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32L_dist ) ) >> 0  ) & 0xff;
		ldist_rom[ l++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32L_dist ) ) >> 8  ) & 0xff;

		rdist_rom[ m++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32R_dist ) ) >> 0  ) & 0xff;
		rdist_rom[ m++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32R_dist ) ) >> 8  ) & 0xff;

		//cross_rom[ n++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32Cross_protect_dist) ) >> 0  ) & 0xff;
		//cross_rom[ n++ ] = ( ( ( Uint16 )( g_secinfo[ i ].int32Cross_protect_dist) ) >> 8  ) & 0xff;
	}
...
void load_line_info_rom( void )
{
	...
	for( i = 0; i < 128; i++ )
	{
		g_secinfo[ i ].int32dist = ( int32 )( ( dist_rom[ j++ ] & 0xff ) << 0 );
		g_secinfo[ i ].int32dist |= ( int32 )( ( dist_rom[ j++ ] & 0xff ) << 8 );
		
		g_secinfo[ i ].int32turn_way = ( int32 )( ( turn_rom[ k++ ] & 0xff ) << 0 );
		g_secinfo[ i ].int32turn_way |= ( int32 )( ( turn_rom[ k++ ] & 0xff ) << 8 );		

		g_secinfo[ i ].int32L_dist = ( int32 )( ( ldist_rom[ l++ ] & 0xff ) << 0 );
		g_secinfo[ i ].int32L_dist |= ( int32 )( ( ldist_rom[ l++ ] & 0xff ) << 8 );

		g_secinfo[ i ].int32R_dist = ( int32 )( ( rdist_rom[ m++ ] & 0xff ) << 0 );
		g_secinfo[ i ].int32R_dist |= ( int32 )( ( rdist_rom[ m++ ] & 0xff ) << 8 );
		
		//g_secinfo[ i ].int32Cross_protect_dist = ( int32 )( ( cross_rom[ n++ ] & 0xff ) << 0 );
		//g_secinfo[ i ].int32Cross_protect_dist |= ( int32 )( ( cross_rom[ n++ ] & 0xff ) << 8 );	
	}
	...
	for( i = 0; i <= g_int32total_cnt ; i++ )		
		TxPrintf("[ %3d ]  DIST : %ld	TRUN : 0x%04x	CROSS  :  %ld \n", i, g_secinfo[ i ].int32dist, ( Uint16 )g_secinfo[ i ].int32turn_way, g_secinfo[i].int32Cross_protect_dist);
	TxPrintf("\n\n");
#endif
}
*/
