...
#define DIV_127_IQ17	_IQ17(127.0)
...
interrupt void SENSOR_ISR() 
{
	...
	AdcRegs.ADCCHSELSEQ2.all = adc_prog[SENSOR_COUNT];
	AdcRegs.ADCCHSELSEQ3.all = adc_prog[SENSOR_COUNT];
	AdcRegs.ADCCHSELSEQ4.all = adc_prog[SENSOR_COUNT];
	
	if(Flag.Sensor_U16)	GpioDataRegs.GPASET.all = sensor_prog[SENSOR_COUNT];
	else;
...
interrupt void ADC_ISR()  
{
	...
	else	
	{
		...
		SenAdc.Div127_U16[SENSOR_COUNT] = IQ_TO_UINT16(_IQmpy(_IQdiv(_IQ(SenAdc.AdcValue_U16[SENSOR_COUNT]),			SenAdc.MaxminusMin_IQ17[SENSOR_COUNT]),		DIV_127_IQ17));
	}
	...
	else	
	{
		...
		SenAdc.Div127_U16[SENSOR_COUNT + 8] = IQ_TO_UINT16(_IQmpy(_IQdiv(_IQ(SenAdc.AdcValue_U16[SENSOR_COUNT + 8]),	SenAdc.MaxminusMin_IQ17[SENSOR_COUNT + 8]),	DIV_127_IQ17)) ;
	}
...
void POSITION_COMPUTE()
{
	...
	if(sum_127div_u16)
	{
		...
		sum_mpy_wd_iq7 += _IQ7mpy(_IQ7(POSITION_WEIGHT_I32[SenAdc.Position_U16_CNT + 0]), _IQ7(SenAdc.Div127_U16[SenAdc.Position_U16_CNT + 0])); 
		sum_mpy_wd_iq7 += _IQ7mpy(_IQ7(POSITION_WEIGHT_I32[SenAdc.Position_U16_CNT + 1]), _IQ7(SenAdc.Div127_U16[SenAdc.Position_U16_CNT + 1]));
		sum_mpy_wd_iq7 += _IQ7mpy(_IQ7(POSITION_WEIGHT_I32[SenAdc.Position_U16_CNT + 2]), _IQ7(SenAdc.Div127_U16[SenAdc.Position_U16_CNT + 2]));
		sum_mpy_wd_iq7 += _IQ7mpy(_IQ7(POSITION_WEIGHT_I32[SenAdc.Position_U16_CNT + 3]), _IQ7(SenAdc.Div127_U16[SenAdc.Position_U16_CNT + 3]));	

		SenAdc.Position_IQ10 = _IQ7div(sum_mpy_wd_iq7, _IQ7(sum_127div_u16)) << 3;
		...
		if((CUR_POSITION > POSITION_WEIGHT_I32[0]))		{ SenAdc.Position_U16_CNT = 0;	SENSOR_STATE_U16_CNT = 8;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[15])	{ SenAdc.Position_U16_CNT = 12;	SENSOR_STATE_U16_CNT = 8;	SENSOR_ENABLE = LEFT_ENABLE; }
		else;
		if(CUR_POSITION > POSITION_WEIGHT_I32[1])		{ SenAdc.Position_U16_CNT = 0;	SENSOR_STATE_U16_CNT = 7;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[14])	{ SenAdc.Position_U16_CNT = 12;	SENSOR_STATE_U16_CNT = 7;	SENSOR_ENABLE = LEFT_ENABLE; }
		else;
		if(CUR_POSITION > POSITION_WEIGHT_I32[2])		{ SenAdc.Position_U16_CNT = 0;	SENSOR_STATE_U16_CNT = 6;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[13])	{ SenAdc.Position_U16_CNT = 12;	SENSOR_STATE_U16_CNT = 6;	SENSOR_ENABLE = LEFT_ENABLE; }
		
		else if(CUR_POSITION > POSITION_WEIGHT_I32[3])	{ SenAdc.Position_U16_CNT = 1;	SENSOR_STATE_U16_CNT = 5;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[12])	{ SenAdc.Position_U16_CNT = 11;	SENSOR_STATE_U16_CNT = 5;	SENSOR_ENABLE = LEFT_ENABLE; }	
		
		else if(CUR_POSITION > POSITION_WEIGHT_I32[4])	{ SenAdc.Position_U16_CNT = 2;	SENSOR_STATE_U16_CNT = 4;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[11])	{ SenAdc.Position_U16_CNT = 10;	SENSOR_STATE_U16_CNT = 4;	SENSOR_ENABLE = LEFT_ENABLE; }
		
		else if(CUR_POSITION > POSITION_WEIGHT_I32[5])	{ SenAdc.Position_U16_CNT = 3;	SENSOR_STATE_U16_CNT = 3;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[10])	{ SenAdc.Position_U16_CNT = 9;	SENSOR_STATE_U16_CNT = 3;	SENSOR_ENABLE = LEFT_ENABLE; }
		
		else if(CUR_POSITION > POSITION_WEIGHT_I32[6])	{ SenAdc.Position_U16_CNT = 4;	SENSOR_STATE_U16_CNT = 2;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[9])	{ SenAdc.Position_U16_CNT = 8;	SENSOR_STATE_U16_CNT = 2;	SENSOR_ENABLE = LEFT_ENABLE; }
		
		else if(CUR_POSITION > POSITION_WEIGHT_I32[7])	{ SenAdc.Position_U16_CNT = 5;	SENSOR_STATE_U16_CNT = 1;	SENSOR_ENABLE = RIGHT_ENABLE; }
		else if(CUR_POSITION < POSITION_WEIGHT_I32[8])	{ SenAdc.Position_U16_CNT = 7;	SENSOR_STATE_U16_CNT = 1;	SENSOR_ENABLE = LEFT_ENABLE; }
		
		else if((CUR_POSITION <= POSITION_WEIGHT_I32[7]) && (CUR_POSITION >= POSITION_WEIGHT_I32[8]))	
...
void START_END_LINE()
{
	...
	{	
		...
		if(Flag.Fast_U16 == ON)		LINE_SECOND(&Search[SECOND_MARK_U16_CNT]);	
		else;																	
	}
	else if((Flag.MoveState_U16 == ON) && (TIME_INDEX_U32 > (PERIOD_1div4SECONDdivCPUTIMER >> 1)))	
	{
		Flag.MoveState_U16 = OFF;	Flag.STOP = ON;
		RMotor.StopAccel_IQ16 = _IQ16mpy(RMotor.Velocity_IQ16, _IQ16div(RMotor.Velocity_IQ16, HEIGHT_ME_IQ22 >> 6));
		LMotor.StopAccel_IQ16 = _IQ16mpy(LMotor.Velocity_IQ16, _IQ16div(LMotor.Velocity_IQ16, HEIGHT_ME_IQ22 >> 6));
		MOVE_TO_END(_IQ17(200.0), _IQ16(0.0), RMotor.StopAccel_IQ16, LMotor.StopAccel_IQ16); 
	}
	else;
}
